...
import com.bol.spinner.ui.bean.SpinnerBusinessData;
...
public class SpinnerDataViewAction extends AnAction {

    private static final Logger logger = Logger.getInstance(SpinnerDataViewAction.class);
    ...
    public void actionPerformed(AnActionEvent e) {
        ...
        try {
           ...
           if (fileName.endsWith(".xls")) {
                ...
                if(endLine == 0){
                    UIUtil.showWarningNotification(project, "Please select a line", "");
                    return;
                }
                String selectLineContent = editor.getDocument().getText(new TextRange(
                        editor.getDocument().getLineStartOffset(endLine),
                        editor.getDocument().getLineEndOffset(endLine)
                ));
               SpinnerBusinessData spinnerBusinessData = new SpinnerBusinessData(firstLineText, selectLineContent);
               SpinnerDataView dataViewWindow = UIUtil.getSpinnerDataViewWindow(project);
               if(dataViewWindow == null){
                   dataViewWindow = UIUtil.getSpinnerDataViewWindow(project);
               }
               dataViewWindow.display(spinnerBusinessData);
           } else {
