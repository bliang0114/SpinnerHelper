...
intellijPlatform {
    pluginConfiguration {
        ...
        changeNotes = """
            ...
            <li>优化 MQL 结果区窗口控制、注释过滤和相关使用说明，补充文档注释中的 <code>@see</code> Java 文件、类及方法引用，并支持 Ctrl/⌘-点击跳转</li>
            <li>为 Spinner 文件定制页签编辑增加卡顿追踪日志，便于定位视图切换和数据构建过程中的性能问题</li>
            <li>为数据表格增加按列筛选功能，提供筛选激活状态视觉指示和右键清除筛选菜单</li>
            <li>在部署、MQL 编辑器、Spinner 编辑器、对象浏览和 Trigger Query 等操作视图中显示当前连接环境</li>
            <li>新增服务器缓存、属性缓存和国际化缓存的手动刷新入口，优化缓存刷新调用方式，并将缓存刷新连接与部署连接隔离</li>
            </ul>
            <h2>3.0.6</h2>
            <ul>
